...
const Navbar: React.FC<NavbarProps> = ({ isAdmin, isGameMode = false }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-colors duration-300 ${containerClass}`}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <button onClick={() => handleNavClick('/')} className="flex items-center gap-2 group z-50 relative">
            ...
            <span className={`font-bold text-lg tracking-tight relative z-10 ${logoClass}`}>
              LAHS <span className={`${subTextClass} font-medium`}>Aviation</span>
            </span>
          </button>
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`px-4 py-2 text-sm transition-all duration-200 font-medium relative group ${
                  isActive(link.path) ? activeTextClass : textClass
                } ${isGameMode ? 'font-mono uppercase tracking-wider' : ''}`}
              >
                {link.name}
                <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 rounded-full transition-all duration-300 ${isGameMode ? 'bg-[#39ff14]' : 'bg-primary'} ${
            ...
            {isAdmin ? (
              ...
              >
                <Compass className="w-3 h-3" /> {isGameMode ? 'LOGIN' : 'Pilot Access'}
              </button>
            )}
          </div>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-40 flex flex-col items-center justify-center space-y-8 transition-all duration-500 md:hidden ${
        isGameMode ? 'bg-black' : 'bg-white'
      } ${
        ...
        <button 
          onClick={() => handleNavClick('/admin')}
          className={`text-lg mt-8 font-mono border px-6 py-2 rounded-lg flex items-center gap-2 ${
            isGameMode 
